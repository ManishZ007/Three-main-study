...
Command: npx gltfjsx@6.5.3 bike.gltf -t 
*/
...
export const BikeModel = (props: JSX.IntrinsicElements["group"]) => {
  const { nodes, materials } = useGLTF("./model/bike.gltf") as GLTFResult;

  // ******************************************************************
  // this way we access the individual mesh and do some operation on that
  // you just need to find the mesh by commenting them
  const frontOuterMesh = useRef<THREE.Mesh>(null);
  const frontInnerMesh = useRef<THREE.Mesh>(null);

  const backOuterMesh = useRef<THREE.Mesh>(null);
  const backInnerMesh = useRef<THREE.Mesh>(null);

  useFrame((state, delta) => {
    if (frontOuterMesh.current && frontInnerMesh.current) {
      frontOuterMesh.current.rotation.y -= delta;
      frontInnerMesh.current.rotation.z -= delta;
    }

    if (backOuterMesh.current && backInnerMesh.current) {
      backOuterMesh.current.rotation.y -= delta;
      backInnerMesh.current.rotation.z -= delta;
    }
  });
  // ******************************************************************
